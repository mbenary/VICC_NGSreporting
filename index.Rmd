...
data %>% 
  ...
  ggbarplot(x = "cancer_type", y = "n", fill = "n", 
...
data %>% 
  ...
  mutate(percReported = Reported / value) -> data.reported
...
data %>% 
  ...
  mutate(sumResponses = sum(Reported, `Not Reported`, na.rm = T),
         `No Response` = value - sumResponses) %>% 
  mutate(percReported = Reported / value) %>% 
  mutate(continent = "Worldwide") %>% 
...
data.essentiality %>% 
  ...
  ggscatter(x = "normEssent", y = "percReported", palette = palContinents, 
            ...
            xlab = "Score of Essentiality", ylab = "% Reported") + 
  lims(x = c(0, 1), y = c(0,1))+
  geom_vline(xintercept = cutoff, color = "#8B0000", lty = "dashed", size = 2) +
