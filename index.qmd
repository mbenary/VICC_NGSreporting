...
data.essentiality |> 
  ...
  ggbarplot(x = "name", y = "normEssent", palette = palContinents, 
            fill = "continent", 
            rotate = T, 
            label = TRUE, lab.size = textSize/6, lab.nb.digits = 2, lab.pos = "in", lab.col = "white",
            lab.vjust = 0.5, lab.hjust = 4,
            xlab = "", ylab = "Score of Essentiality", position = position_dodge(0.9)) + 
#  ylim(-0.16, 1)+
  geom_hline(yintercept = cutoff, color = "#8B0000", lty = "dashed", size = 1) +
  theme_pubr(base_size = textSize/2) + 
  theme(legend.position = "top", legend.title=element_blank()) -> plot.overview
plotly::ggplotly(plot.overview)# |> plotly::layout(xaxis = list(hoverformat = '.2f'))
```
...
#  ylim(-0.16, 1)+
  ...
  theme_pubr(base_size = textSize/2) + 
  theme(legend.position = "top", legend.title=element_blank()) -> plot.variants
plotly::ggplotly(plot.variants)
```
...
data.essentiality |> 
  ...
  geom_hline(yintercept = cutoff, color = "#8B0000", lty = "dashed", size = 2) +
  theme_pubr(base_size = textSize/2) + 
  theme(legend.position = "top", legend.title=element_blank()) -> plot.functional
plotly::ggplotly(plot.functional) 
```
