...
data.essentiality |> 
  ...
  ggbarplot(x = "continent", y = "normEssent", palette = palContinents, 
            fill = "continent", facet.by = "name", ncol=1, scales = "free_y",
            rotate = T, 
...
data |> 
  ...
  mutate(Essentiality = factor(Essentiality, levels = responses.essential)) -> data.response.essentiality

data.response.essentiality |> 
  filter(section == "Overview") |> 
  group_by(continent, name) |> 
  count(Essentiality, name = "count") |> 
  mutate(name = factor(name, levels = orderOverview$name)) |> 
  ggbarplot(y = "count", x = "continent", fill = "continent", alpha = "Essentiality", facet.by = "name", palette = palContinents, ncol = 1,
            rotate = T, order = names(palContinents), #panel.labs = list(name = rev(orderOverview$label)),
            ylab = "# Participants", xlab = "") + 
  theme_pubr(base_size = textSize/2) +
  theme(legend.position = "none") -> p2

plotly::subplot(margin = 0.07, plotly::ggplotly(p2) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "# Participants")), plotly::ggplotly(plot.overview, tooltip = c("y", "fill")) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "Score of Essentiality")), titleX = TRUE, titleY = TRUE)
...
plotly::subplot(margin = 0.07, plotly::ggplotly(p2) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "# Participants")), plotly::ggplotly(plot.variants, tooltip = c("y", "fill")) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "Score of Essentiality")), titleX = TRUE, titleY = TRUE)
...
plotly::subplot(margin = 0.07, plotly::ggplotly(p2) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "# Participants")), plotly::ggplotly(plot.functional, tooltip = c("y", "fill")) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "Score of Essentiality")), titleX = TRUE, titleY = TRUE)
```
...
data.essentiality |> 
  ...
  ggbarplot(x = "continent", y = "normEssent", palette = palContinents, 
            ...
            xlab = "", ylab = "Score of Essentiality", position = position_dodge(0.9)) + 
  geom_hline(yintercept = cutoff, color = "#8B0000", lty = "dashed", linewidth = 1) +
  theme_pubr(base_size = textSize/2) -> plot.treatment
...
plotly::subplot(margin = 0.07, plotly::ggplotly(p2) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "# Participants")), plotly::ggplotly(plot.treatment, tooltip = c("y", "fill")) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "Score of Essentiality")), titleX = TRUE, titleY = TRUE)
```
...
data.essentiality |> 
  ...
  ggbarplot(x = "continent", y = "normEssent", palette = palContinents, 
            ...
            xlab = "", ylab = "Score of Essentiality", position = position_dodge(0.9)) + 
  geom_hline(yintercept = cutoff, color = "#8B0000", lty = "dashed", linewidth = 1) +
  theme_pubr(base_size = textSize/2) -> plot.trial
...
plotly::subplot(margin = 0.07, plotly::ggplotly(p2) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "# Participants")), plotly::ggplotly(plot.trial, tooltip = c("y", "fill")) |> plotly::layout(showlegend = FALSE, xaxis = list(title = "Score of Essentiality")), titleX = TRUE, titleY = TRUE)
```
...
data.essentiality |> 
  filter(section == "Overview") |> 
  left_join(data.reported, by = c("group", "section", "name", "continent")) -> df

cor_labels <- df |>
  group_by(continent) |>
  summarise(
    r   = cor(normEssent, percReported, use = "complete.obs"),
    pval = cor.test(normEssent, percReported)$p.value,
    .groups = "drop"
  ) |>
  mutate(
    label = paste0("r = ", round(r, 2), ", p = ", signif(pval, 2))
  )

annotations <- lapply(seq_len(nrow(cor_labels)), function(i) {
  list(
    x         = 0.2,
    y         = max(df$percReported, na.rm = TRUE) * (0.99 - (i - 1) * 0.02),
    text      = paste0("<b>", cor_labels$continent[i], ":</b> ", cor_labels$label[i]),
    showarrow = FALSE,
    xref      = "x",
    yref      = "y",
    font      = list(color = palContinents[i], size = 12)
  )
})

df |> 
  plotly::plot_ly(x = ~normEssent, y = ~percReported, colors=palContinents, type = 'scatter', mode = 'markers', text=~name,
  # Hover text:
  hovertemplate = paste0(
      "<b>%{text}</b><br>",
      "Essentiality: %{x:.2f}<br>",
      "Reported: %{y:.2f%}<extra></extra>"
    ),
...
data.reported |> 
  ...
  pivot_longer(cols = all_of(responses.reported), values_to = "count", names_to = "report") |> 
